...
## Activating the LLM (Ollama / SGLang / OpenAI)

The gateway defaults to `LLM_PROVIDER=mock` — canned text from each agent's answer template. Even with `OPENAI_API_KEY` ambient in your shell, the gateway stays inactive until you opt in.

**Status check:** `GET /api/health/llm` reports `{provider, configured, active}`.

**Laptop (Ollama + Qwen2.5):**

```bash
# one-time: cache the model outside the broken /Volumes/NVME_FAST symlink
OLLAMA_MODELS="$HOME/.ollama-models" ollama pull qwen2.5:3b

# every dev session
OLLAMA_MODELS="$HOME/.ollama-models" ollama serve &        # in another terminal
LLM_PROVIDER=ollama OLLAMA_BASE_URL=http://127.0.0.1:11434 OLLAMA_MODEL=qwen2.5:3b bin/dev
```

**Production (`bcc-ap-llm01` → SGLang on `bcc-ap-infer01`):**
...
**Cold-start fallback (OpenAI, only if county egress is allowlisted):**

```bash
LLM_PROVIDER=openai OPENAI_API_KEY=sk-... bin/dev
```

**Auto-discovery (sglang > ollama > openai > mock):**

```bash
LLM_PROVIDER=auto bin/dev
```

When the LLM is active, every `compose` step in `/api/query` and `/api/agent/dispatch` streams real model tokens (still grounded on the same retrieved citations from the BM25 corpus). The SSE shape doesn't change — the UI doesn't care.
