...
CREATE TABLE corpus_chunks (
    ...
    source_type TEXT NOT NULL,         -- "ldc" | "ordinance" | "fl_ag_opinion" | "internal_opinion" | "procedure"
...
**RRF merge** (Cormack 2009): `score(d) = Σ 1 / (k + rank_in_list_i(d))` — k=60 is the empirical sweet spot.
...
Both L3 and L4 emit `event_kind="rag_hit"` per spec §6 ROI table. The `_EMIT_DEFAULTS` constant carries workflow, tool="rls_apex", surface="other", task_type="search", dept, role_band. `hit_count` lives in `extra`. The HybridRetriever class doesn't emit; the tool entry point does. This keeps the retrieval library testable in isolation.
...
Polls every MCP tool's `/health` endpoint on a 30s timer, aggregates into a single response.
...
TOOL_HEALTH_ENDPOINTS = {
    ...
    "validate_rls_structure":         "http://127.0.0.1:30102/health",
    "extract_fields":                 "http://127.0.0.1:30103/health",  # legacy port from v0.2.0a
...
| W4 | Stub opinion authoring | 18 stubs covering 6 matter types; each includes realistic PII + privilege spans for redaction pipeline |
| W5 | Redaction pipeline LLM detector prompt + audit write contract | Test: detector returns spans + reasons; audit rows are written before redacted text is applied |
| W6 | Calendar working-days scraper + check | Test: holidays loaded from corpus; weekend handling; cross-year boundaries |
