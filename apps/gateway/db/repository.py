...
class PgRepo:
    ...
    def __init__(self, pool: asyncpg.Pool) -> None:
        ...
        self._breaker = CircuitBreaker(
            name="rls_pg_write", failure_threshold=5, open_duration_seconds=30.0
        )
        ...
        async def _genesis() -> str:
            async with self._pool.acquire() as c:
                try:
                    ...
                    # Designed idempotency replay. UNIQUE(idempotency_key) collision
                    # is a healthy PG signal, NOT a breaker-countable failure. The
                    # failed tx is rolled back by the transaction-context exit; the
                    # connection is now back to autocommit on the same `c`.
